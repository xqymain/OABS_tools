...
class Guahao(object):
    ...
    def get_patient_id(self, doctor):
        ...
        if isinstance(doctor, str):
            #logging.error ("没号了，亲～")
            #sys.exit(-1)
            return # 无号退出逻辑由上级函数 run () 负责
        addr = self.gen_doctor_url(doctor)
        response = self.browser.get(addr, "")
        ret = response.text
        m = re.search(u'<div class=\\"personnel active\\" name=\\"(?P<patientId>\d+)\\"[^>]*><div class=\\"imgShow\\"></div><div class=\\"infoRight\\"><span class="name">' + self.config.patient_name, ret)
        if m is None:
            sys.exit(" 获取患者 ID 失败 ")
        else:
            self.config.patient_id = m.group('patientId')
            logging.info(" 患者 ID:" + self.config.patient_id)
